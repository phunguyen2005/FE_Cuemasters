        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => handleFilterChange('NoShow')}
                className={`px-4 py-2 text-sm font-bold transition-colors ${
                  filter === 'NoShow'
                    ? 'rounded-full bg-on-surface text-surface'
                    : 'rounded-full bg-surface-container-low text-secondary hover:bg-surface-container-high'
                    ...
                    <div className="flex-1">
                      <div className="mb-2 flex items-center gap-3">
                        ...
                        <p className="font-headline text-lg font-bold">
                          {booking.tableName ? `Bàn ${booking.tableName}` : `${booking.requestedTableType === 'Pool' ? 'Bida lỗ' : booking.requestedTableType === 'Snooker' ? 'Snooker' : 'Carom'} (Sẽ xếp bàn khi tới)`}
                        </p>
                      </div>
                      <div className="mb-4 flex flex-wrap items-center gap-3 text-sm text-secondary">
                        ...
                        <span>&bull;</span>
                        <span className="font-medium">{(booking.actualCost || booking.totalPrice).toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
                        <span>&bull;</span>
                        <span className={`rounded-sm px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest ${
                          booking.status === 'NoShow' ? 'bg-secondary/10 text-secondary' :
                      ...
                      {booking.depositAmount !== undefined && (
                        <div className="flex flex-wrap items-center gap-2 text-xs text-secondary mt-2">
                          <span className="material-symbols-outlined text-[14px]">payments</span>
                          <span>Cọc: {booking.depositAmount.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
                          
                          {booking.status === 'Cancelled' && (
                            <span className={booking.depositForfeited ? 'text-error font-medium' : 'text-emerald-600 font-medium'}>
                              ({booking.depositForfeited ? 'Không hoàn cọc' : 'Đã hoàn cọc'})
                            </span>
                          )}
                          
                          {booking.status === 'NoShow' && (
                            <span className="text-error font-medium">(Mất cọc)</span>
                          )}
                        </div>
                      )}
                    </div>
